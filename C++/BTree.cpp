...
void Node::split(int i, std::shared_ptr<Node> y) {
    // novo nó que vai receber metade das chaves de y
    auto z       = std::make_shared<Node>(y->is_leaf);
    z->key_count = MIN_KEYS;

    // copiar as últimas m/2 chaves de y para z
    for (int j = 0; j < MIN_KEYS; j++)
        z->keys[j] = y->keys[j + MIN_KEYS + 1];

    if (y->is_leaf == false)                 // se y não é folha
        for (int j = 0; j <= MIN_KEYS; j++)  // copia os filhos de y pra z
            z->children[j] = y->children[j + MIN_KEYS + 1];

    y->key_count = MIN_KEYS;  // reduz o número de chaves de y

    // move os filhos do nó pra dar espaço ao novo filho
    for (int j = key_count; j >= i + 1; --j)
        children[j + 1] = children[j];

    children[i + 1] = z;  // inserir novo nó z como filho

    // mover as chaves pra dar espaço da nova chave
